...
class ImageDataset(Dataset):
    ...
    def __init__(self, dataset, transform_ori=None, transform_mask=None, training=False, change_background=False):
        ...
        if self.change_background:
            self.mask_list = glob(
                '/media/data/ai-city/Track2/AIC21_Track2_ReID/AIC21_Track2_ReID/track2_segmented/mask/*.npy')
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            if self.transform_ori is not None and self.transform_mask is not None:
                img_ori = self.transform_ori(img)
                img_mask = self.transform_mask(img)
            return img_ori, img_mask, pid, camid, domain, img_path
        else:
