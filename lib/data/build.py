...
from .samplers import RandomIdentitySampler, MPerClassSampler, RandomIdentityCrossDomainSampler  # New add by gu
from .transforms import build_transforms


def make_data_loader(cfg, shuffle_train=True):
    train_transforms_ori, train_transforms_mask = build_transforms(cfg, is_train=shuffle_train)
    ...
    print(f'num_train:{num_train_pids}')
    # import ipdb; ipdb.set_trace()
    num_classes = num_train_pids
    train_set = ImageDataset(dataset.train, train_transforms_ori, train_transforms_mask, training=True, change_background=False)
    # import ipdb; ipdb.set_trace()
